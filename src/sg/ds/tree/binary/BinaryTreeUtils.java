...
public class BinaryTreeUtils {
	...
	public static void main(String[] args) {
		...
		Node<String> root = buildStringTree("1 2 3 4 5 6 7 8 9");
		printTree(root);
	}
	...
	public static Node<String> buildStringTree(String input) { // simple utility function to build
		String[] dataArr = input.split(" ");
		Node<String> root = null;
		for (int i = 0; i < dataArr.length; i++) {
			if (root == null) {
				root = new Node<String>(dataArr[i]);
	...
	private static <T> void insertData(Node<T> currNode, T data) {
		if (data == null || currNode == null) {
			return;
		}

		if (currNode.left == null) {
			currNode.left = new Node<T>(data);

		} else if (currNode.right == null) {
			currNode.right = new Node<T>(data);
